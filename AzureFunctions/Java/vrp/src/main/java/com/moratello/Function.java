...
public class Function {
    ...
    public HttpResponseMessage run(
        ...
        context.getLogger().info("Vehicle Routing Problem solver.");
        ...
        if (request.getBody().isPresent()) {
            ...
            return request.createResponseBuilder(HttpStatus.BAD_REQUEST).body("You must POST a VRP instance file!").build();
        }
        ...
        if (request.getQueryParameters().get("vns") == null) {
            return request.createResponseBuilder(HttpStatus.BAD_REQUEST).body("The parameter 'vns' must be defined!").build();
        } else {
            vns = Boolean.parseBoolean(request.getQueryParameters().get("vns"));
        }

        if (request.getQueryParameters().get("k_opt") != null) {
            k_opt = Integer.parseInt(request.getQueryParameters().get("k_opt"));
        }

        if (request.getQueryParameters().get("n_iter") != null) {
            n_iter = Integer.parseInt(request.getQueryParameters().get("n_iter"));
        }
