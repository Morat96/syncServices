...
exports.lambdaHandler = async (event, context) => {
    try {
        ...
        console.log(event['queryStringParameters']['vns']);
        console.log(event['queryStringParameters']['k_opt']);
        console.log(event['queryStringParameters']['n_iter']);

        if (!event['queryStringParameters']['vns']) {
            return {
                'statusCode': 400,
                'body': JSON.stringify({
        ...
        vns = event['queryStringParameters']['vns'];

        if (event['queryStringParameters']['k_opt']) {
            k_opt = event['queryStringParameters']['k_opt'];
        }
        if (event['queryStringParameters']['n_iter']) {
            n_iter = event['queryStringParameters']['n_iter'];
        }
        ...
        fs.writeFileSync('/tmp/instance.txt', content);
        ...
        var file = fs.readFileSync("/tmp/instance.txt").toString().split("\n");
