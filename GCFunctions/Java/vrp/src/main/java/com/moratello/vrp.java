...
public class vrp implements HttpFunction {
    @Override
    public void service(HttpRequest request, HttpResponse response)
        ...
        if(!request.getFirstQueryParameter("vns").isPresent()) {
            response.setStatusCode(500);
            output.addProperty("Error", "The parameter vns must be defined!");
            response.getWriter().write(output.toString());
            return;
        } else {
            System.out.println("VNS: " + request.getFirstQueryParameter("vns").get());
            vns = Boolean.parseBoolean(request.getFirstQueryParameter("vns").get());
        }

        if(request.getFirstQueryParameter("k_opt").isPresent()) {
            k_opt = Integer.parseInt(request.getFirstQueryParameter("k_opt").get());
        }

        if(request.getFirstQueryParameter("n_iter").isPresent()) {
            n_iter = Integer.parseInt(request.getFirstQueryParameter("n_iter").get());
        }
