...
ibmcloud wsk package update --shared yes benchdbJS -a description "A benchmark for CouchDB databases. Designed for Openwhisk, written in Javascript." \
...
cd createBench
zip -r -q action.zip *
ibmcloud wsk action update benchdbJS/createBench action.zip --kind nodejs:10 --web true
cd ..

echo "Creating DELETE action: deleteBench"
cd deleteBench
zip -r -q action.zip *
ibmcloud wsk action update benchdbJS/deleteBench action.zip --kind nodejs:10 --web true
cd ..

echo "Creating PUT action: updateBench"
cd updateBench
zip -r -q action.zip *
ibmcloud wsk action update benchdbJS/updateBench action.zip --kind nodejs:10 --web true
cd ..

echo "Creating GET action: readBench"
cd readBench
zip -r -q action.zip *
ibmcloud wsk action update benchdbJS/readBench action.zip --kind nodejs:10 --web true
cd ..
